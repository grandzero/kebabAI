...
app_dir=$(pwd)
target_path="$app_dir/extensions/kebabai-submodule/extensions/vscode"
link_path="$app_dir/extensions/kebabai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/kebabai-submodule" ]; then
    echo "Removing existing kebabai-submodule directory"
    execute "rm -rf $app_dir/extensions/kebabai-submodule" "Failed to remove existing kebabai-submodule directory"
fi
...
execute "cd ./extensions/kebabai-submodule" "Failed to change directory to extensions/kebabai-submodule"
